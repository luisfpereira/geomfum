...
In Deep Functional Maps, the functional map is computed by the forward pass computed on given descriptors.
...
class ForwardFunctionalMap(abc.ABC):
    ...
    def _compute_functional_map(self, sdescr_a, sdescr_b, mask):
        ...
        for i in range(mask.shape[0]):
            if self.lmbda == 0:
                ...
                C =  np.linalg.inv(AA_aa + self.lmbda * MASK_i) @ AA_ba[ i, :].reshape(-1, 1)
            C_i.append(C.T)   
    ...
    def __call__(self, mesh_a, mesh_b, descr_a, descr_b):
        ...
        mask = self._compute_mask(mesh_a.basis.vals, mesh_b.basis.vals, self.resolvent_gamma)
        fmap_12 = self._compute_functional_map(sdescr_a, sdescr_b, mask)
        
        if self.bijective:
            mask = self._compute_mask(mesh_b.basis.vals, mesh_a.basis.vals, self.resolvent_gamma)
    ...
    def _compute_mask(self, evals_a, evals_b, resolvant_gamma):
        ...
        M_re = evals_gamma_b / (np.square(evals_gamma_b) + 1) - evals_gamma_a / (np.square(evals_gamma_a) + 1)
