...
class PointNetDescriptor(Descriptor):
    ...
    def __init__(self, n_features=128,device=torch.device('cpu'), feature_transform=False):
        super(PointNetDescriptor, self).__init__()
        self.model = PointNet(k= n_features, feature_transform=feature_transform).to(device)
    ...
    def __call__(self, mesh):
        ...
        with torch.no_grad():
            point_cloud = torch.tensor(mesh.vertices, dtype=torch.float32, device=self.device)
