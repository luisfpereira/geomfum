...
class DijkstraAlgorithm:
    ...
    def __init__(
        ...
        n_clusters=4,
        clusterer=None,
        neighbors_regressor=None,
        n_jobs=1,
    ):
        # n_clusters ignored if clusterer is not none
        if clusterer is None:
            # TODO: also pass max_iter?
            clusterer = KMeans(n_clusters=n_clusters, max_iter=10)

        if neighbors_regressor is None:
            # NB: radius is updated in the code
            neighbors_regressor = RadiusNeighborsRegressor(
                radius=1.01, algorithm="ball_tree", leaf_size=40, n_jobs=1
            )

        self.clusterer = clusterer
        self.neighbors_regressor = neighbors_regressor
        self.n_jobs = n_jobs

    def update_radius(self, radius):
        self.neighbors_regressor.radius = radius

    def _build_adjacency_matrix(self, mesh):
        N = mesh.n_vertices

        # TODO: need to add to Mesh
        edges = mesh.edges

        I_base = edges[:, 0]  # (p,)
        J_base = edges[:, 1]  # (p,)
        V_base = np.linalg.norm(
            mesh.vertices[J_base] - mesh.vertices[I_base], axis=1
        )  # (p,)

        return sparse.csc_matrix((V_base, (I_base, J_base)), shape=(N, N))
    ...
    def compute_sparse_dijkstra_mp(
        ...
        """
        # real_rho -> initial_rho?

        # Divide the samples in n_clusters parts
        self.clusterer.fit(mesh1.vertices[subsample])

        # Build adjacency matrix
        graph = self._build_adjacency_matrix(mesh1)

        # Fit a ball tree to the vertices
        self.update_radius(1.01 * rho)
        self.neighbors_regressor.fit(mesh1.vertices, np.ones(mesh1.n_vertices))

        # TODO: make it a bit more agnostic?
    ...
    def compute_sparse_dijkstra(
        ...
        # TODO: non mp version: create two versions with nice __new__

        # TODO: how to incorporate update_sampled_points?
        ...
        # Fit a ball tree to the vertices
        self.neighbors_regressor.radius = 1.01 * rho
        self.neighbors_regressor.fit(mesh1.vertices, np.ones(mesh1.n_vertices))

        seen = np.full(mesh1.n_vertices, False)

        iterable = range(self.clusterer.n_clusters)
        if verbose:
            iterable = tqdm(iterable)

        for class_index in iterable:
            subsample_sub_test = self.clusterer.labels_ == class_index  # (m,)
            ...
            res_test = self.neighbors_regressor.radius_neighbors(
...
class LocalFunctionsConstructor:
    """
    Parameters
    ----------
    ...
    def __init__(self, dist_ratio=3.0):
    ...
    def build_local_mat_new(
        ...
        rho = self.dist_ratio * avg_radius

        N = mesh1.n_vertices
        n_samples_base = subsample.size

        # TODO: use dijkstra here?
        edges = mesh1.edges
...
class Transform:
    ...
    def local_f(self, vals, rho, chi="exp", return_zeros_indices=False):
        ...
        # TODO: index selection goes outside
...
class ScalableLaplacianSpectrumFinder(LaplacianSpectrumFinder):
    ...
    # TODO: improve inheritance; need to add abstract class
    ...
    def __init__(
        ...
        if local_func_constr is None:
            # TODO: update
            local_func_constr = LocalFunctionsConstructor()

        if transform is None:
            transform = exp_compact
    ...
    def __call__(self, shape):
        ...
        """
        # TODO: should input simply be a shape or a subsampled shape?
        # TODO: SubsampledShape idea may have potential
        # TODO: i.e. rlow is more than a PointCloud

        return self.evals, self.evecs

    def get_approx_spectrum(self, Wb, Ab, verbose=False):
        """
        Eigendecoposition using Wbar and Abar
        """
        # TODO: transform `verbose` in logger, move everything there
        # TODO: does it need to be a function?
        if verbose:
            print(f"Computing {self.spectrum_size} eigenvectors")
            start_time = time.time()

        eigenvalues, eigenvectors = self.eig_solver(Wb, M=Ab)
    ...
    def build_red_matrices(
        ...
        # TODO: replace by call to LocalFunctionsConstructor
...
class ScalableHierarchicalMesh(HierarchicalShape):
    ...
    def _scalable_fm(self, mesh, sampler):
        ...
        # TODO: commented this line because unnecessary?
